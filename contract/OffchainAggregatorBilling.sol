...
contract OffchainAggregatorBilling is Owned {
  ...
  struct Billing {
    ...
    // Pay transmitter back this much PLI per unit eth spent on gas
    // (1e-6PLI/ETH units)
    uint32 microLinkPerEth;

    // Fixed PLI reward for each observer, in PLI-gwei units
  ...
  // PLI-wei-denominated reimbursements for gas used by transmitters.
  //
  // This is always one greater than the actual value, so that when the value is
  // reset to zero, we don't end up with a zero value in storage (which would
  // result in a higher gas cost, the next time the value is incremented.)
  ...
  /*
   * @notice emitted when the PLI token contract is set
   * @param _oldLinkToken the address of the old PLI token contract
   * @param _newLinkToken the address of the new PLI token contract
   */
  event LinkTokenSet(
  ...
  /*
   * @notice sets the PLI token contract used for paying oracles
   * @param _linkToken the address of the PLI token contract
  ...
  /*
   * @notice gets the PLI token contract used for paying oracles
   * @return linkToken the address of the PLI token contract
   */
   ...
   * @param microLinkPerEth reimbursement per ETH of gas cost, in 1e-6PLI units
   * @param linkGweiPerObservation reward to oracle for contributing an observation to a successfully transmitted report, in 1e-9PLI units
   * @param linkGweiPerTransmission reward to transmitter of a successful report, in 1e-9PLI units
   */
  event BillingSet(
    uint32 maximumGasPrice,
    uint32 reasonableGasPrice,
   ...
   * @param _microLinkPerEth reimbursement per ETH of gas cost, in 1e-6PLI units
   * @param _linkGweiPerObservation reward to oracle for contributing an observation to a successfully transmitted report, in 1e-9PLI units
   * @param _linkGweiPerTransmission reward to transmitter of a successful report, in 1e-9PLI units
  ...
  /**
   * @notice emitted when an oracle has been paid PLI
   * @param transmitter address from which the oracle sends reports to the transmit method
   * @param payee address to which the payment is sent
   * @param amount amount of PLI sent
   * @param linkToken address of the PLI token contract
   */
  event OraclePaid(
    ...
    internal
  {
    ...
    if (linkWeiAmount > 0) {
      ...
      // Poses no re-entrancy issues, because PLI.transfer does not yield
      // control flow.
   ...
   * @param _amount maximum amount to withdraw, denominated in PLI-wei.
   * @dev access control provided by billingAccessController
   */
  ...
  /**
   * @notice allows oracles to check that sufficient PLI balance is available
   * @return availableBalance PLI available on this contract, after accounting for outstanding obligations. can become negative
   */
    ...
    internal
  {
    ...
    // microLinkPerEth is 1e-6PLI/ETH units, gasCostEthWei is 1e-18ETH units
    // (ETH-wei), product is 1e-24PLI-wei units, dividing by 1e6 gives
    // 1e-18PLI units, i.e. PLI-wei units
