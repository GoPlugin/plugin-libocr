...
contract OCR2Aggregator is OCR2Abstract, OwnerIsCreator, AggregatorV2V3Interface {
  ...
  struct Transmitter {
    ...
    // by the transmitter plus additional rewards. The entire PLI supply (1e9
    // PLI = 1e27 Juels) will always fit into a uint96.
    uint96 paymentJuels;
  }
  ...
  struct HotVars {
    ...
    // Plugin Aggregators expose a roundId to consumers. The offchain reporting
  ...
  /**
   * @param link address of the PLI contract
   ...
   * @param juelsPerFeeCoin exchange rate between feeCoin (e.g. ETH on Ethereum) and PLI, denominated in juels
  ...
  /***************************************************************************
   * Section: Configurable PLI Token
   **************************************************************************/
   ...
   * @param oldLinkToken the address of the old PLI token contract
   * @param newLinkToken the address of the new PLI token contract
   */
  event LinkTokenSet(
   ...
   * @param linkToken the address of the PLI token contract
   ...
   * @param amount maximum amount to withdraw, denominated in PLI-wei.
   * @dev access control provided by billingAccessController
   */
  ...
  // Total PLI due to participants in past reports (denominated in Juels).
